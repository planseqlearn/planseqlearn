...
from disrep4rl.models.decoder import SacAeDecoder
from disrep4rl.models.encoder import DrQV2Encoder
...
class V4Agent:
    ...
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 mask_loss_coeff, detach_critic, detach_decoders):
        self.device = device
        ...
        self.mask_decoder = SacAeDecoder(in_channels=16,
                                         out_channels=obs_shape[0] // 3,
                                         output_act=nn.Sigmoid(),
                                         repr_dim=int(feature_dim /
                                                      2)).to(device)
        self.decoder = SacAeDecoder(in_channels=16,
                                    out_channels=obs_shape[0],
                                    repr_dim=int(feature_dim / 2)).to(device)
    ...
    def update_critic_and_decoders(self, obs, encoded_obs, action, reward,
        ...
        f = self.critic.trunk(encoded_obs)
        f1 = f[:, :int(f.shape[1] / 2)]
        f2 = f[:, int(f.shape[1] / 2):]
