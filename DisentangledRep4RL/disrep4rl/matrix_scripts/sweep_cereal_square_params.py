        ...
        for camera_name in camera_names:
            for env_name in env_names:
                for _ in range(num_seeds):
                    ...
                    cmd = f"python disrep4rl/train.py agent=drqv2 use_proprio=False use_wandb=True save_video=True num_train_frames=1000000 wandb.project_name=mprl wandb.run_name={run_name} task={env_name} mprl=True path_length=50 experiment_id={run_name} matrix=True seed={seed} {params}"
                    os.system(cmd)
                    num_exps += 1
                    time.sleep(10)
