...
for idx, env_name in enumerate(env_names):
    ...
    for _ in range(num_seeds):
        ...
        run_name = f"drqv2_e2e_baseline_final_{env_name}_{valid_obj_names}"
        cmd = f"python disrep4rl/train.py experiment_subdir=e2e_baseline_final agent=drqv2 use_wandb=True seed={seed} debug=True save_video=True num_train_frames=5000000 eval_every_frames=50000 num_eval_episodes=10 wandb.project_name=mprl_paper_results wandb.run_name={run_name} task={env_name} valid_obj_names={valid_obj_names} mprl=False path_length=500 action_repeat=2 experiment_id={run_name} matrix=True"
        os.system(cmd)
        time.sleep(5)