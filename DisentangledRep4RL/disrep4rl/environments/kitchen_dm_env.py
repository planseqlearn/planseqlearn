...
from disrep4rl.environments.wrappers import (
    ...
    SlimMaskWrapper, StackRGBAndMaskWrapper, Wrist_Camera_Render_Wrapper, get_env_action_spec,
    get_env_observation_spec)


class Kitchen_Wrapper(dm_env.Environment):

    def __init__(self, env_name: str, discount=1.0, seed=None, path_length=280, mprl=False):
        self.discount = discount
        self._env = gym.make(env_name)
        if mprl:
            from rlkit.mprl.mp_env_kitchen import MPEnv
            self._env = MPEnv(self._env, use_vision_pose_estimation=False, teleport_instead_of_mp=False)
        # self._env.seed(seed)
        self.env_name = env_name
        self.fixed_view_physics = Camera_Render_Wrapper(self._env.sim,
                                             lookat=[-0.3, .5, 2.],
                                             distance=1.86,
                                             azimuth=90,
                                             elevation=-60)
        self.physics = Wrist_Camera_Render_Wrapper(self._env.sim,
                                             lookat=[-0.3, .5, 2.],
                                             distance=1.86,
                                             azimuth=90,
                                             elevation=-60)
    ...
    def reset(self) -> dm_env.TimeStep:
        ...
        self.current_step = 0
        self.episode_total_score = 0
        observation = self._env.reset(get_intermediate_frames=True)
    ...
    def step(self, action: int) -> dm_env.TimeStep:
        ...
        observation, reward, done, info = self._env.step(action, get_intermediate_frames=True)
        self.current_step += 1
...
def make_kitchen(name,
    ...
    env = Kitchen_Wrapper(env_name=name, discount=discount, seed=seed, path_length=path_length, mprl=mprl)
