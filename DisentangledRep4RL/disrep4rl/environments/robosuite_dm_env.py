...
from disrep4rl.environments.metaworld_custom_envs import (MT3_Customized,
                                                          MT5_Customized,
                                                          MT10_Customized,
                                                          MT_Door)
...
class Robosuite_Wrapper(dm_env.Environment):
    ...
    def __init__(self,
        ...
        if mprl:
            self._env = MPEnv(self._env, **mp_env_kwargs)
    ...
    def reset(self) -> dm_env.TimeStep:
        ...
        if self.use_proprio:
            new_observation['state'] = np.concatenate((observation['robot0_eef_pos'], observation['robot0_eef_quat']))
        if self.use_fixed_plus_wrist_view:
    ...
    def step(self, action: int) -> dm_env.TimeStep:
        ...
        observation, reward, d, info = self._env.step(action, get_intermediate_frames=True)
        self.current_step += 1
    ...
    def observation_spec(self) -> specs.BoundedArray:
        ...
        if self.use_fixed_plus_wrist_view:
            pixels_obs_space = np.concatenate((pixels_obs_space, pixels_obs_space), axis=2)
    ...
    def action_spec(self) -> specs.BoundedArray:
        ...
        spec = specs.BoundedArray(shape=self._env.action_spec[0].shape,
...
def make_robosuite(name,
    ...
    env = Robosuite_Wrapper(env_name=name,
                     discount=discount,
                     seed=seed,
    ...
    env = ActionRepeatWrapper(env,
                              action_repeat, use_metaworld_reward_dict=True)
